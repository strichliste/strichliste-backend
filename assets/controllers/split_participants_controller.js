...
export default class extends Controller {
  ...
  static values = { rowLabel: String, removeLabel: String, previewLabel: String, currency: String };
  ...
  updatePreview() {
    ...
    const raw = (this.element.querySelector('#amount')?.value || '').trim().replace(',', '.');
    const total = Number.parseFloat(raw);
    const people = this.rows().filter((r) => r.querySelector('.split-invoice-pick')?.value).length;
    ...
    this.previewTarget.textContent = this.previewLabelValue.replace('%share%', share);
