...
prod: ## Build and start the production stack (requires APP_SECRET in .env)
	@$(DOCKER_COMP) -f compose.yaml -f compose.prod.yaml pull --ignore-buildable
	@$(DOCKER_COMP) -f compose.yaml -f compose.prod.yaml build --pull
	@$(DOCKER_COMP) -f compose.yaml -f compose.prod.yaml up -d --wait --wait-timeout 120
